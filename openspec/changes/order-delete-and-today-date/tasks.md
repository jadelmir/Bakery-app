...
- [ ] 4.1 Add or update authenticated desktop/mobile browser coverage to open an order, confirm deletion, verify it disappears, reload, and verify it remains absent; cover New Order showing the current local date. Trace: persisted deletion and date-default scenarios. The deployed staging build showed the current local date (`2026-08-17`) and the accessible delete confirmation on an authenticated order. Full create/delete/reload acceptance remains open: the default mock backend is non-persistent, local real-backend coverage lacks Supabase credentials, and the current in-app browser session became non-clickable while advancing the hosted multi-step form.
...
- [ ] 4.3 Deploy through staging, complete authenticated delete/reload and today's-date acceptance, update `openspec/PROGRAM_MAP.md` and relevant docs with evidence, then leave hosted rollout open if any gate is not verified. GitHub Pages deployment run `31995967055` succeeded for commit `8f544de`; hosted date/delete affordance checks passed, while the authenticated persistence journey remains open as recorded in 4.1.
