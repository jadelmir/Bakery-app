...
The application SHALL define workspace destinations in one route registry, SHALL synchronize the rendered feature with the browser URL and history, and SHALL keep direct-entry routes behind authentication and explicit active-bakery selection.

**Trace:** PRD F1 URL-based navigation; owner `establish-shared-application-foundation`; upstream `frontend-authentication-shell` and active `add-multi-store-workspaces` selection boundary.

#### Scenario: Using browser history
- **WHEN** a user navigates between two registered bakery features and then activates browser Back or Forward
- **THEN** the URL and rendered feature move together without remounting an unauthorized bakery workspace

#### Scenario: Opening a feature URL without an active bakery
- **WHEN** a user directly opens a registered workspace URL before authentication or bakery selection is complete
- **THEN** the existing auth and bakery-selection gates run before the requested feature can render

#### Scenario: Opening an unknown workspace URL
- **WHEN** an authenticated user with an active bakery opens an unregistered workspace path
- **THEN** the application renders a safe not-found or fallback route with navigation to a registered destination
