...
- [x] 2.3 Add or update desktop/mobile Playwright coverage to create an authenticated manual order, observe it immediately in Orders, reload, and observe it again with its task plan. Evidence: authenticated staging journey passed on 2026-08-17; the new Yuri / Loaf order appeared as the fourth current order immediately after creation and remained visible after a full reload.
...
- Hosted tasks 1.2, 2.4, 3.3, 3.4, 3.5, and the remaining release verification stay open until the complete acceptance gates are captured; no hosted secret values were guessed or committed.
- Authenticated staging reproduction on `https://jadelmir.github.io/Bakery-app/` on 2026-08-17: after the corrective frontend was deployed, creating the available `Yuri` / `Loaf` order redirected to `/orders`, showed `4 current orders` immediately, and showed the same order after a full reload. The persisted order was `095f5691-9512-475c-86b4-1822838fded1`.
- Staging deployment `d374ff3` successfully applied migrations, deployed the Edge Functions, and set the hosted `APP_URL`. A synthetic invitation to `elmirjad+staging-20260817@gmail.com` then rendered `Invitation sent`, showed one pending invitation, and was revoked for cleanup. Invitations to `example.com` addresses correctly surfaced delivery failure and left no pending invitation. Acceptance and Auth redirect-list verification remain outstanding.
